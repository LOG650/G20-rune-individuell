...
1. **Makkerpar-prinsippet:** Den operative prosedyren definerer to operatører (RØD + GUL) som standard for én hendelse. Erlang-C modellerer én server per anrop og undervurderer dermed kapasitetstapet ved simultane hendelser.

2. **Kapasitetsbinding utover samtaletid:** Etter at samtalen er avsluttet (RØD-fasen) er operatøren bundet i GUL-funksjon: utalarmering, samband og oppdragsoppfølging. Bindingstiden er vesentlig lenger enn samtaletiden. Erlang-C behandler denne bindingen som null.

3. **Uavhengige servere:** M/M/c forutsetter at servere er uavhengige. I 110-kontekst er operatørene dynamisk koblet gjennom prosedyrens rollestruktur — RØD og GUL er komplementære, ikke parallelle.
